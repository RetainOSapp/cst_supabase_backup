-- Remove only the additive natural-query overloads; historical RPCs remain.

do $$
begin
  if exists (
    select 1 from public.security_rollout_history rollout
    where rollout.version > '20260714020000'
      and (rollout.migration_name like 'beacon_%' or rollout.migration_name like 'ai_feature_%')
  ) then
    raise exception 'Roll back later Beacon/AI slices before natural query filters';
  end if;
end $$;

delete from public.security_rollout_history where version = '20260714020000';
