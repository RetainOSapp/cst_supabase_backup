...
check("disabled automation preview is empty and run denies before materialization", /if \(!pipelineEnabled\)[\s\S]{0,260}enabled: false[\s\S]{0,160}Pipeline is disabled for this company/i.test(automation) && automation.indexOf("if (!pipelineEnabled)") < automation.indexOf('rpc(\n      "generate_due_renewal_pipeline_items"'));
