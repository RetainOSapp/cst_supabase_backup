...
check("disabled Pipeline preview is empty and run denies before bounded materialization", /if \(!pipelineAccess\.enabled\)[\s\S]{0,700}enabled: false[\s\S]{0,500}Pipeline is disabled for this company/i.test(automation) && automation.indexOf("if (!pipelineAccess.enabled)") < automation.indexOf('rpc("preview_renewal_pipeline_cohort"'));
