...
const checks = [
  ...
  ["company entitlement lookup", manage, /\.from\("company_ai_feature_entitlements"\)[\s\S]+feature_key", "call_analysis"/],
  ["company entitlement effective window", manage, /effective_from[\s\S]+effective_until/],
  ["lightweight entitlement access action", manage, /action === "access"[\s\S]+featureEnabled/],
  ["disabled company actions fail closed", manage, /if \(!featureEnabled\)[\s\S]+not enabled for this company/],
  ...
  ["transcript is untrusted", participantContext, /UNTRUSTED_UTTERANCE_RECORDS_JSON/],
