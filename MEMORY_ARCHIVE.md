...
## Archived From Active Memory - 2026-07-23 Call Intelligence compaction
...
- 2026-07-23 superseded Call Intelligence demo checkpoint (moved verbatim from `MEMORY.md`): isolated `codex/call-intelligence-demo` worktree/branch from current `origin/main`; frontend-only sample dashboard/detail is integrated as the default authenticated `/call-ai` tab and exposed at sample-data-only `/call-intelligence-preview` for Loom QA. It makes no Supabase/provider call; real schema, prompts, ingestion, analysis, entitlements, and migration parity remain unbuilt.
...
- 2026-07-28 active checkpoints: MM MIA auto-offboard follow-ups are live and Jay verified all 62 in Daily Pulse. The block is company-gated by both the MIA policy and an enabled template; title/instructions/action are configurable. Daily Pulse loads exact task-linked clients beyond Supabase's 1,000-row cap. Client deletion now preserves the immutable assignment-history tombstone while removing the client from RetainOS operational views/analytics; direct URLs cannot fall back to stale mirror data. Shanique Messado's partial deletion was reconciled and audited: archived=true, dashboard exclusion=true, zero client-search or Lorcan-churn visibility. General deletion fix awaits production push/QA.
...
## Archived From Active Memory - 2026-08-03
...
- 2026-08-03 MM renewal repair: production DB now treats migrated `current_summary` contracts as effective current evidence and provides atomic service-only summary reconciliation. Exactly 103 fully blank summaries caused by app-side contract edits were restored with before/after audits; rerun finds 0 safe repairs. Partial (406), ambiguous (3), unsupported blank (1), and Lisa Fuller pending confirmation remain untouched. Adam production cohorts verify July 13 including Daniel/Richardson, August 6 without Lisa, and September 7/7 including Lisa. Frontend canonical card/list and UTC calendar-date release awaits production push/QA.
- 2026-08-04 MM canonical renewal frontend is live through `f02f692`: the card and Active Clients Up For Renewal drawer now call the same actor-scoped cohort. Vince's August production cohort verifies 7 eligible / 1 retained / 6 active unresolved, and public bundle `index-BiOWpdvE.js` contains the corrected call. The 103 audited summary repairs remain stable; Lisa Fuller still awaits date confirmation.
...
## Archived From Active Memory - 2026-08-07
