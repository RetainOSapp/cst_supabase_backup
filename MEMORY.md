...
- 2026-07-24 active checkpoints: Call Intelligence tab/API now require effective `call_analysis` entitlement (ES pilot only; Moves has none), while the Pipeline role-access/one-time-scan candidate remains isolated in `/private/tmp/cst-pipeline-gate-f-release` with no migration, deploy, company-setting change, push, or production release.
