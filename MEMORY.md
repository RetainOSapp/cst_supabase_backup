...
- 2026-08-04 MM Call AI reconciliation candidate: manual search includes nonarchived offboarded profiles and all three client-email fields; automatic summary/transcript matching prefers one active match, then permits exactly one offboarded fallback. A full production audit currently finds 23 open MM events: 16 safe unique offboarded email matches and 6 without an exact app-owned email match (the remainder is a non-client/group event). Frontend, four Edge functions, tests, production push, and Jay/Jhoyce QA remain.
