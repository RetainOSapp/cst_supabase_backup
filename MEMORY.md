...
- Retention denominator workspace: `/private/tmp/cst-retention-denominator-fix` on `codex/retention-denominator-completions`, isolated from clean production `origin/main`; keep it local until migration/runtime QA is approved.
...
- 2026-08-06 renewal denominator correction checkpoint: local candidate restores normally completed/offboarded contracts when actual offboard date is on/after contract end, while early churn, Paused, Suspended/MIA, undated legacy offboards, archived rows, and analytics exclusions remain excluded. MM July read-only evidence is 334 eligible / 48 retained / 50 active unresolved / 14%; one of 237 restored normal completions (Dean Bair) renewed the following day. Current production is 97/47. No production apply or push yet; remote CLI dry-run is blocked by the repo's pre-existing legacy migration-history mismatch.
