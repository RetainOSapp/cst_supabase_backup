...
- Retention denominator workspace: `/private/tmp/cst-retention-denominator-fix` on `codex/retention-denominator-completions`; its reversible production correction is applied and runtime-verified, and the branch is the clean rollout record.
...
- 2026-08-06 pause reconciliation release is live: migration `20260805170000` (`f1b8672df74c56edbed860b301dfa6c51c0736a215c800eaeefa5cdc5a242be6`) adds the exact-contract pause ledger and atomic planned-versus-actual calendar-day reconciliation; `manage-client-status` and `manage-company-customization` are deployed JWT-on. Backfill created 45 open evidence-only ledger rows (44 MM, 1 Sales Kick) without changing dates; 79 incomplete/ambiguous current pauses remain untouched. Static contract 13/13, build, and anonymous 401 probes pass. Historical corrections, including Bobbi/Reggie, remain team-workbook-gated.
- 2026-08-07 successor-contract retention correction is production-applied through reversible migrations `20260807170000`–`20260807180000`. Candidate-bound, non-add-on successor contracts are first-class evidence; early renewals stay in contract-end month and late renewals move to successor-start month. MM July active postflight is 86 eligible / 56 retained / 30 unresolved versus 86 / 47 / 39 before; Don Morehouse, Danielle Mein, Caroline Lai, and Rafiq Barnes are July retained, while Monica Vigil is August retained. No client/contract rows changed; source commit and Jay/MM UI QA remain.
...
- 2026-07-24 Call Intelligence ES authenticated-manual pilot is live through `b713b33`: management v2 JWT-on; DB 70/70, Edge 63/63, frontend 30/30, tests/build/browser pass. ES is the only entitlement with a one-time $1 hard cap at $0 used; global config v2 active. No webhook token, calls, transcripts, runs, usage, provider spend, or Zapier automation exists. Production excludes the DEV fixture. Jay's first known-client upload is next; keep its raw transcript until pilot approval, then decide ongoing retention.
- 2026-07-24 MM dashboard lifecycle reconciliation is live in the database: foundation `20260724110000` (`bfed1f6f6c278e2863e1063922bca94c55e367038c0ebc11812a9fd824c1ad3e`) and MM scope `20260724113000` (`813e249944f718c08727522a45b227c25cba7ac1926beb8b6eed2583f8e817a5`). It preserves but excludes 86 Jhoyce-owned and 261 unresolved/inactive-primary clients from MM analytics, corrected 324 safe CST offboard months plus Christopher, reconciled 4 MIA/offboard contradictions, recovered 65 MIA timers, and auto-offboarded 60 due clients with zero failures. Nine later app-owned mismatches remain review-only. Frontend Paused/exclusion release and live UI QA remain.
- 2026-08-06 renewal denominator correction is live through reversible migrations `20260806100000` (`af2d2f980f162c15d38a1d04709a3ff18f92d27f87699d849b15e6c17b05ba56`) and churn guard `20260806103000` (`fe8ce21df5d60e6abcad3799f0363562b38bceef99258b577bcf02f2466cdc90`). Normal completions remain eligible; early/churn-coded offboards, Paused, Suspended/MIA, undated legacy offboards, archived rows, and analytics exclusions remain out. MM July postflight: 334 eligible / 48 retained / 50 active unresolved / 14%, zero expected missing, 1.4s canonical read.
- 2026-08-04 MM Call AI offboarded-profile matching is live through `191338b`: manual search includes nonarchived offboarded profiles and all three client-email fields; automatic summary/transcript matching prefers one active match, then exactly one offboarded fallback. Function versions are call-summary v15, integration-review v12, call-intelligence ingest v2, and management v5; JWT boundaries probed correctly. The live bundle contains the new search. A full production audit finds 23 open MM events: 16 safe unique offboarded email matches, 1 ambiguous active match, and 6 without an exact app-owned email match. Jay/Jhoyce authenticated UI QA remains.
- 2026-08-05 MM navigation performance release is Jay-QA-approved as “way faster.” Commit `adf8f99` route-splits primary pages; uses actor-scoped Dashboard aggregates before fallback/drilldowns; scopes Daily Pulse, Tasks, Calendar, and Resources before transfer; and batches CSM Report history. Build, 11 focused assertions, renewal/churn regressions, and calendar-date verification pass; no database/index change is included.
- 2026-08-05 client-interaction release checkpoint: Jay approved Quick Update and Client Outcomes locally. Six Edge Functions are live with prior JWT/token boundaries preserved; management anonymous probes return 401 and Fathom rejects a missing integration token before intake storage. Feature commit `809d9a2` plus its rollout note is released through production `main`; no company interaction metadata, Pipeline movement, or 12-row MM historical repair has been applied.
- 2026-08-05 Outcomes call attendance is live through `fd00ace`: Calls & Reviews adds the Jay-QA-approved branded Record Call form for attended/missed, configured call type, and optional note through the existing `manage-client-quick-update` boundary. It refreshes the client, History, totals, and recent interactions immediately; no migration/backend deploy was needed. Verification is 13/13, the production build passes, Vercel reached Ready, and the `app.retainos.ai` Client Detail bundle contains the approved UI.
