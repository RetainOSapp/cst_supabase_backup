...
- 2026-07-24 Call Intelligence V1 local candidate on `codex/call-intelligence-v1`: baseline/evidence-v1/evidence-v2 cost $2.592327 total. Quality review found three useful outputs plus two participant-role collisions, one invalid next-step citation, and two weak archetypes. Quality-v3 quarantines collisions pre-provider, drops next steps with invalid citations, suppresses weak archetypes, persists only sanitized structured text, and adds collision/injection tests; zero-cost replay passed 3/3 eligible calls with 44/44 supported citations and quarantined two. Semantic next-step review remains required. Gates: Edge 20/20, eval 5/5, DB 54/54, source 56/56, frontend 19/19, build. A three-call quality-v3 plus one synthetic adversarial paid retest needs fresh approval ($0.94 rounded ceiling). No production migration/function/frontend deploy, token, entitlement, allowance, or Zap change occurred.
