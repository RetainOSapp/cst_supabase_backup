...
- 2026-08-04 local performance candidate is isolated on `codex/mm-navigation-performance` and is not pushed/deployed. It route-splits primary pages; uses actor-scoped Dashboard aggregates before fallback/drilldowns; scopes Daily Pulse, Tasks, Calendar, and Resources before transfer; and batches CSM Report history. Build, 11 focused assertions, renewal/churn regressions, and calendar-date verification pass. Jay's authenticated MM navigation QA is the release gate; no database/index change is included.
