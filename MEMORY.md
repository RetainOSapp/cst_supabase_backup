...
- 2026-08-05 Outcomes call-attendance local candidate is isolated on `codex/outcomes-call-attendance`: Calls & Reviews adds a branded Record Call form for attended/missed, configured call type, and optional note through the existing `manage-client-quick-update` boundary. It refreshes the client, History, totals, and recent interactions immediately; no migration or backend deploy is needed. Verification is 13/13 and the production build passes. Local QA is at `http://127.0.0.1:5173/`; saving there writes real production data, so use an intended client event. Port 5174 used a separate browser session and was retired after a false request-dispatch failure.
