...
- 2026-07-24 active checkpoints: Call Intelligence ES-only pilot first call + buy-in prompt cost $0.14388. Native Fathom `0:00` timestamps had caused zero parsed utterances; parser fix is live, Aron Lucas was user-confirmed and mapped client-side, and a zero-cost replay now resolves all 164 utterances (92 client, 72 team, 0 unknown). On-demand runs auto-refresh. Reprocess this existing call once before adding calls. Pipeline MM preview uses current-contract evidence and complete pagination: 4,686 checked, 683 eligible in the configured 30-day catch-up + 45-day lead window, 3 already tracked, 7 Pipeline rows unchanged, 0 automation runs; recurring scheduling stays off.
