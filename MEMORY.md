...
- 2026-07-23 Call Intelligence V1 local candidate: secure intake/dedupe/reconciliation, six-table RLS model, exact Glide baseline plus structured V2, budgeted AI worker/on-demand prompts, actor-scoped real UI, DEV-only fixture, tests, and rollout/Zapier docs are built on `codex/call-intelligence-v1`. Gates pass: Edge 16/16, eval 3/3, DB 53/53, source 35/35, frontend 19/19, build, and 1440px/320px browser QA. Official docs confirm GPT-5.6 Luna/Terra/Sol IDs and Responses schema; `openai-standard-2026-07-23` prices are pinned with standard tier and implicit cache writes disabled. The ignored five-call corpus plans 45 Terra-medium calls with a conservative $5.26 ceiling. Jay approved that paid eval, but the first attempt stopped before any provider call/cost because `OPENAI_API_KEY` is empty in local `.env.graphify`; add a server-only key locally, then rerun. No production migration/function/frontend deploy, token, entitlement, allowance, Zap change, paid call, or provider call occurred.
