...
- 2026-07-24 active checkpoints: Call Intelligence ES-only pilot first call + buy-in prompt cost $0.14388. Native Fathom `0:00` timestamps had caused zero parsed utterances; corrected replay resolves 164 with client/team/unknown roles, and on-demand runs now auto-refresh. Reprocess this existing call once before adding calls. Pipeline role access/one-time scan is live at `ac2f5b1`; MM is Director-only, recurring renewal scheduling remains off, and runtime proof changed zero Pipeline items/runs.
