...
- 2026-07-27 active checkpoints: Call Intelligence ES manual-pilot spend is $0.385236; one real Fathom/Zapier webhook delivery is next. Pipeline release `ee2fb5c` is live through migration `20260727130000` and MM gates `20260727132000`/`20260727133000`. Jay's 12-card bounded QA passed; all 12 were audited-assigned to exact active primary owners. MM alone is enrolled in daily one-day Renewal recurrence with no first backfill, a hard 25-card cap, and task/offboarding automation off. First run `383e36e2-656b-4734-8f90-1fa45c39ede5` created 12 owned cards with 0 failures; MM now has 24 visible real cards and next runs 2026-07-28 UTC. Both renewal and early-activation crons are singular/active. Do not enroll another company before observing MM. Advocacy note-only saves plus nested-note History rendering are live through Quick Update v13, Outcomes v11, and production main; Ben/Sales Kick retest remains. MM MIA auto-offboard follow-up migration `20260727140000` (`19d3aee6287fafd53da8f53615d98c8a6ee9d744ce6e49006797e48b1a5eb700`) and customization v28 are live: one editable template backfilled 62/62 assigned tasks with 0 failures and 5 fallbacks. The follow-up block is company-gated by both the MIA auto-offboard policy and an enabled trigger template; its Daily Pulse title/instructions/action are template-configurable. Initial UI QA exposed Supabase's 1,000-row client cap: all 62 task clients were outside the first page and were incorrectly removed by the visibility intersection. Daily Pulse now loads exact task-linked clients before filtering. Jay QA path is View as Moves Method → Daily Pulse.
