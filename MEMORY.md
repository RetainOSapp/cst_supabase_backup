...
- 2026-08-05 MM MIA dropoff-label release is live through `05204ab`: stable reason `auto_suspended_timeout` remains active/counts as churn, its company label is `Dropoff — MIA timeout`, future automation preserves company-edited labels, and Client Details/History expose exit outcome and reason. All 65 existing reason-linked offboards remain intact; no client/history backfill was needed. Vercel is Ready and the live bundle contains the new UI.
