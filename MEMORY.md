...
- 2026-07-24 active checkpoints: Call Intelligence tab/API require effective `call_analysis` entitlement (ES only; Moves off); first ES call succeeded (77,941 chars, Terra, $0.09492) and result readback was corrected to canonical `company_members.legacy_glide_row_id`. Pipeline role access/one-time scan is live at `ac2f5b1`; MM is Director-only, recurring renewal scheduling remains off, and runtime proof changed zero Pipeline items/runs.
