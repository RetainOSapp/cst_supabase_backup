...
- 2026-07-23 Call Intelligence V1 local candidate: secure intake/dedupe/reconciliation, six-table RLS model, Glide prompt seed plus structured V2, automatic budgeted AI dispatch/worker, on-demand prompts, actor-scoped real UI, DEV-only responsive fixture, tests, and rollout/Zapier docs are built on `codex/call-intelligence-v1`. Automated gates and 1440px/320px browser QA pass. No production migration/function/frontend deploy, token, entitlement, allowance, Zap change, paid model eval, or provider call occurred.
