...
- `[~]` `[qa]` Call Intelligence V1 local release candidate.
  ...
  - Automated evidence passes: Edge/provider/dispatch 18/18, eval scorer/harness 4/4, DB/dependency/rollback contract 54/54, Edge/source security 47/47, frontend/security/privacy 19/19, production build, and browser QA at 1440px/320px. Official OpenAI docs confirm Luna/Terra/Sol API identifiers, strict Responses structured output including string `pattern`, and Terra-medium as the balanced starting point. Model-scoped `openai-standard-2026-07-23` pricing is pinned; calls force standard tier and disable implicit cache writes.
  ...
  - Evidence-v2 correction is local and green: `structured_v2_evidence_v2` sends explicit timestamped utterance records plus a sanitized speaker-role map, requires timestamp/role/quote to come from one record, forces unmatched/ambiguous speakers to remain `unknown`, machine-enforces 4–12 whitespace-separated quote words with strict-schema `pattern`, keeps timestamp/role fail-closed validation, and mechanically synchronizes the migration seed. The focused retest is five structured-only calls with a conservative 1,176,566-micro ($1.18 rounded) ceiling; it has not run. Production remains unchanged. [qa]
  - Jay QA remains production-pilot-only after separately approved disabled DB/function/frontend rollout and the corrected private-corpus retest: one known-client call, replay/dedupe, unknown and multi-client reconciliation, structured output, one on-demand prompt, role scope, hard-budget/global-pause denial, and rollback. The private baseline used paid provider calls; no production provider dispatch or production change occurred.
