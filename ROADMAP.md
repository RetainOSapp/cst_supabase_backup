...
- `[~]` `[qa]` Call Intelligence V1 local release candidate.
  ...
  - Automated evidence passes: Edge/provider/dispatch 18/18, eval scorer/harness 4/4, DB/dependency/rollback contract 54/54, Edge/source security 43/43, frontend/security/privacy 19/19, production build, and browser QA at 1440px/320px. Official OpenAI docs confirm Luna/Terra/Sol API identifiers, strict Responses structured output, and Terra-medium as the balanced starting point. Model-scoped `openai-standard-2026-07-23` pricing is pinned; calls force standard tier and disable implicit cache writes.
  ...
  - Evidence correction is local and green: `structured_v2_evidence_v1` permits one short uninterrupted quote per claim, forbids stitched/paraphrased evidence, validates the quote inside its cited utterance, supplies sanitized trusted participant roles, rejects timestamp-speaker attribution mismatches, and mechanically synchronizes the migration seed with the evaluated prompt/schema. The focused retest is five structured-only calls with a conservative 1,122,829-micro ($1.13 rounded) ceiling; it has not run. Production remains unchanged. [qa]
