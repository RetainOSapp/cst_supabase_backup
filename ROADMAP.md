...
- `[~]` `[qa]` Observe the Pipeline release with MM for one week before broad rollout or advanced automation/reporting.
  ...
  - 2026-08-06 Sam-feedback correction candidate is complete locally on `codex/pipeline-sam-feedback`. Renewal preview now synchronizes the board to the same custom Renewal date range, explains that 100 is a per-run write safeguard, and supports repeated safe batches. Daily Pulse treats renewal/SR dates as calendar dates. Strategic Review completion atomically creates a missing eligible contract-linked item directly in Review Complete, and daily recurrence rechecks the configured catch-up-through-lead horizon under the existing 25/company and 100/global caps. Read-only MM evidence: 4,869 evaluated, 502 currently eligible (344 August), 210 visible cards (43 August), and 13 legitimate completed SRs with no card; all 13 are repairable and 11 stored due dates carry the old one-day timezone shift. The exact 13-record repair gate is frozen. Build plus 18/18 correction, 56/56 foundation, 47/47 workflow, and 27/27 rollout checks pass. `[~]` `[release-gate]` No migration, Edge deploy, frontend release, scheduler run, repair write, commit, or push has occurred; next is forced-rollback SQL/runtime proof, then the separately approved production release and Jay/Sam QA.
