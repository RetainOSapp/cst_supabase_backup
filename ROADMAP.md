...
- `[~]` `[qa]` Vercel dotted-route refresh fix.
  - 2026-07-14 diagnosis: the SPA fallback excluded every path containing a dot, but Glide client/table IDs can contain dots; refreshing those routes returned Vercel `404 NOT_FOUND` before React loaded. Local `vercel.json` now excludes only static Vite assets and `favicon.svg`. Deploy from `main`, then have Adam refresh a dotted Client Detail URL to close.
