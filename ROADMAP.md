...
- `[~]` `[qa]` Call Intelligence V1 local release candidate.
  ...
  - Automated evidence passes: Edge/provider/dispatch 16/16, eval scorer 3/3, DB/dependency/rollback contract 53/53 (including immutable price lineage/database cost recomputation), Edge/source security 35/35, frontend/security/privacy 19/19, production build, eval dry-run, and browser QA at 1440px/320px with no horizontal overflow. Official OpenAI documentation confirms Luna/Terra/Sol API identifiers, strict Responses structured output, and Terra-medium as the balanced starting point. The model-scoped `openai-standard-2026-07-23` price card is pinned; calls force standard service tier and disable implicit cache writes so metering cannot silently drift. The five real Zap runs are a git-ignored 253,045-character private corpus; Terra-medium plans 45 paid calls with a conservative $5.26 ceiling, but none were executed.
  - Jay approved the 45-call Terra-medium paid evaluation on 2026-07-23. The first execution attempt stopped before provider dispatch and cost $0 because the local `.env.graphify` has an empty `OPENAI_API_KEY`; add a server-only local key and rerun the same bounded command. Production remains unchanged.
