...
- `[~]` `[polish]` Retention Percentage.
  ...
  - 2026-08-06 correction candidate (supersedes the 2026-08-04 active-only denominator): normally completed clients remain renewal-eligible when their actual offboard date is on/after the relevant contract end. Early churn, Paused, Suspended/MIA, undated offboards, archived rows, and analytics exclusions remain out. Read-only MM July evidence is 47 currently counted retained + 50 active unresolved + 237 restored normal completions; one restored completion (Dean Bair) renewed the following day, producing 334 eligible / 48 retained / 14%. The current live active-only scope returns 97/47. Migration and rollback are local-only pending apply/runtime QA; the remote CLI dry-run is blocked by the repo's pre-existing legacy migration-history mismatch, not by this migration. [qa]
