...
- `[~]` `[qa]` Call Intelligence V1 Ethical Scaling manual pilot.
  ...
  - 2026-07-24 paid private baseline: all 45 Terra-medium requests completed for $1.849210 versus the approved $5.26 ceiling. Structured V2 used 86.2% fewer input tokens, cost 75.5% less, and had 48.0% lower cumulative provider latency than the eight-prompt legacy flow; schema validity was 5/5. Promotion failed correctly: only 146/197 evidence quotes occurred inside their cited timestamp context and no call passed the all-evidence hard gate.
  - 2026-07-24 evidence-v1 retest: exactly 5/5 Terra-medium calls completed with no retries for $0.372006. It failed promotion safely: 55/67 unique evidence items were timestamp-grounded, 42/67 were correctly attributed, and 0/5 calls passed runtime validation. Aggregate analysis found 11/14 exact speaker-label matches, one safe unique first-name match, and two genuinely unknown labels; Terra improperly inferred roles for some unknown labels. [qa]
  - 2026-07-24 evidence-v2 and quality audit: the five-call retest completed without retries for $0.371111; raw output passed 3/5 with 65/67 citations supported. Transcript-level review found three useful outputs, two calls made unusable by conflicting participant identities, one next step with an invalid citation, and two overconfident archetypes. `structured_v2_quality_v3` now quarantines role collisions before a provider request, omits next steps whose citations fail structural/grounding/role checks, suppresses weak archetypes, stores only sanitized structured text, and adds synthetic collision/injection coverage. Semantic next-step review remains required. A zero-provider-call replay passed 3/3 eligible calls with 44/44 supported citations and quarantined two. Total evaluation spend remains $2.592327. Production remains unchanged. [qa]
  - 2026-07-24 quality-v3 promotion passed: the approved three eligible private calls plus one synthetic adversarial request cost $0.286526 against the $0.94 ceiling. Private output passed 3/3 with 42/42 retained citations supported; two collision calls were quarantined at zero cost. The adversarial request passed 1/1 with 4/4 citations and all six injection-resistance expectations. Independent semantic review scored every eligible dimension 4–5/5, found conservative archetypes and appropriate dates, and reported no P0-P3 blocker. Total evaluation spend across 59/59 requests is $2.878853. [qa]
  ...
  - 2026-07-24 the secure manual-upload release is live through `b713b33`; management v2 remains JWT-on and anonymous upload denies. Policy release `20260724160000`, zero-usage rounding correction `20260724161000`, and ES-only activation `20260724162000` are applied. Ethical Scaling is the only `call_analysis` pilot, with a one-time $1 hard cap at $0 used. No webhook token, calls, transcripts, runs, usage, provider spend, or Zapier automation exists yet. Production now lazy-loads the DEV fixture so its dummy data is absent from the live bundle. [qa]
  - 2026-07-24 company visibility correction: the Call Intelligence tab and every actor-scoped management action now require an effective `call_analysis` company entitlement. SuperAdmin → SaaS Clients → company → AI Features exposes the released Call Intelligence start/pause/disable and USD hard-cap controls. Existing Call AI reconciliation remains independent. Moves Method has no entitlement; Ethical Scaling remains the only pilot. [qa]
  - 2026-07-24 first live ES manual call processed successfully: one 77,941-character check-in transcript produced one succeeded Terra run for $0.09492 with no processing error. The initial result page failed to load because its assigned-member reader requested nonexistent `company_members.glide_row_id`; the correction uses canonical `legacy_glide_row_id` for list/detail and CSM assignment scope. [qa]
  - 2026-07-24 first-result quality diagnosis: native Fathom timestamps were `0:00`–`1:07:01`, while the production parser accepted only `00:00:00`; all 164 utterances therefore fell back to unattributed raw text, producing insufficient sentiment and only one supported next step. The parser fix is live; after Jay confirmed Aron Lucas is client-side and his mapping was added, a zero-cost replay resolves all 164 utterances as 92 client and 72 team-member, with none unknown. The buy-in on-demand run succeeded with `🟢` for $0.04896, but the detail UI stayed on `claimed`; active runs now auto-refresh and succeeded results render green. [qa]
  - 2026-07-24 corrected reprocess succeeded for $0.101098 and produced positive sentiment, 3 supported next steps, and 20/28; cumulative pilot spend is $0.244978. Jay approved the overall analysis improvement but rejected the blinking poll UX, permanently queued success notice, and single cherry-picked sentiment quotes. The frontend correction keeps existing detail mounted during background polling, tracks the exact queued run through completion, auto-settles notices, locks run controls against duplicate spend, and labels sentiment as aggregate full-call tone/confidence while retaining clickable evidence in specific findings. Frontend verifier 47/47 and build pass; production UI QA remains. [qa]
  - 2026-07-24 second-call manual QA passed: fresh base analysis cost $0.074508 and the selected on-demand prompt cost $0.065750; Jay reported both fast and useful. The on-demand finalizer exposed a call-level state bug and the transcript label `Angus Parker (Ali Abdaal)` exposed a trusted parenthetical-alias gap. Current data was repaired without a provider call. Production migration `20260724180000` (`91427810eb0f0898c159c9cbaf1fc1fa70291e46688eba93a52c75017af04e36`) now restores the completed base state after terminal on-demand runs, and the worker maps exact parenthetical client-account aliases. Zero-cost replay resolves all 129 utterances as 65 client and 64 team-member. Gates pass DB 77/77, Edge 70/70, provider 14/14, frontend 47/47, and build. Cumulative ES manual-pilot spend is $0.385236. [qa]
  - Authenticated manual upload, base analysis, reprocessing, transcript evidence navigation, and one on-demand prompt have passed Jay QA. The next controlled milestone is one real Fathom/Zapier webhook delivery; automatic replay/dedupe, unknown and multi-client reconciliation, role scope, hard-budget/global-pause denial, retention decision, and rollback remain.
...
- `[x]` 2026-06-21 Advocacy tracking QA passed.
  ...
  - 2026-07-27 note-history correction: note-only advocacy context now saves without changing asked/received counts, and Client History renders/searches both standalone notes and notes attached to advocacy actions. Existing action-attached notes are recovered from stored history payloads; note-only text discarded before this correction cannot be reconstructed. `manage-client-quick-update` v13, `manage-client-outcomes` v11, and the frontend release are live; Ben/Sales Kick retest remains. [qa]
...
- `[~]` `[qa]` Vercel dotted-route refresh fix.
  - 2026-07-14 diagnosis: the SPA fallback excluded every path containing a dot, but Glide client/table IDs can contain dots; refreshing those routes returned Vercel `404 NOT_FOUND` before React loaded. Production `vercel.json` now excludes only static Vite assets and `favicon.svg`; direct refresh of a dotted Client Detail URL returns 200. Adam should confirm in-browser to close.
...
- `[x]` Pipeline Phases 0-4, early-renewal scheduling, and the Gate F frontend release are live.
  ...
  - ES is visible; MM was disabled after the video setup. Sales Kick's master gate and its Renewal/Expansion definitions are enabled, but automation is off and both pipelines have zero items. Viewer access remains off and there are zero pending scheduled activations.
  ...
  - 2026-07-22 Expansion starter repair: the starter had combined `value_source=fixed` with a null default, so every save correctly failed validation but the browser showed only the generic Edge error. Sales Kick's one invalid starter was repaired to `none` and enabled with automation off; future Expansion starters use the valid `none` default, and Pipeline configuration now surfaces the server's actionable error message.
- `[~]` `[qa]` Observe the Pipeline release with MM for one week before broad rollout or advanced automation/reporting.
  ...
  - 2026-07-24 renewal-preview correction: migration `20260724173000` (`068243db70c112d6032a4493692ed86e3aeeb4437187624928942871fc960542`) now evaluates one current fixed-term contract per client, accepts migrated `unknown` cadence only with matching current-contract evidence, rejects missing/ambiguous evidence, and detects existing items by source contract or renewal date. The API paginates beyond 1,000 rows and the UI discloses exact counts, configured timing window, eligible names, and exclusion breakdown before enabling materialization. MM read-only evidence: 4,686 checked, 683 eligible in its 30-day catch-up + 45-day lead window, 3 already tracked, 7 total Pipeline rows unchanged, and 0 automation runs.
  - 2026-07-27 bounded rollout release `5c7fe9e` is live. Migrations `20260727120000` (`2e0511ff1d20d25eb0191cb8b8d30a94ad9629e0aa17c3f9efc4d4b7964dc92b`) and `20260727121000` (`d05f3f5706bcd73b96ffc2cf9f61af1fe601eb1f1a3088b0a5b3977227290f55`) add default-paused scheduler controls, separate first-backfill/recurring enrollment, service-only actor-bound 15-minute preview tokens, hard date/cap binding, and read-only Admin status. Production runtime proof exercised empty/capped MM cohorts, actor/expiry/staleness denial, idempotent consume, scheduler isolation, and evidence-aware rollback inside a forced-rollback transaction. Live readback: global and renewal materialization paused, 0 enrollment rows, 0 tokens, new cron registered but inert, and the existing early-renewal cron singular/active. The five untouched Gate C3 video-demo rows were audited-archived; MM now has 0 visible items and awaits Jay's capped real-data QA run.
  - 2026-07-27 Jay's capped MM production QA passed at 12 cards: exact dates/exclusions, cap-change invalidation, materialization, drawer/card display, and refresh persistence. Release `ee2fb5c` plus migration `20260727130000` (`c415a9ee90fe1b27593aac4e7a51b53f497daaa6244aff2a6f6fd1d36b0b3332`) now assigns contract-linked Renewal cards only through one exact active primary-member mapping and caches verified company/role sidebar visibility in session storage while the server revalidates. Gate `20260727132000` (`18f0cc28ef69bbc464d1264daff9a9f13ce9646ff04c29c8aae593d72fdde268`) audited-assigned all 12 QA cards. A temporary fully restored MM Director proof allowed workspace/configuration and all 12 items while denying the SuperAdmin-only run.
  - 2026-07-27 MM-only recurrence gate `20260727133000` (`06aa893ca0a0c81108504adfac93cf805f52a41b5afd63031a39b03768ce2a71`) is live: no first backfill, one calendar-day cohort, hard 25-card company cap, task/offboarding automation off, and no other company enrolled. First observed run `383e36e2-656b-4734-8f90-1fa45c39ede5` completed with 12 created, 0 skipped, 0 failures; all 12 inherited owners. MM has 24 visible real contract-linked/owned cards and next run is 2026-07-28 UTC. The new renewal cron and independent early-renewal cron are each singular/active.
  - 2026-07-28 Pipeline filter polish release `df73909` is live. Workspace now includes an independent **Secondary assignee** filter sourced from active company members referenced by the scoped clients, matching both app-owned and legacy assignment IDs while leaving the existing Pipeline owner filter unchanged. MM production evidence at release: 44 visible cards, five populated secondary-assignee options, and three cards without a secondary assignment retained under **All secondary assignees**. Jay's Loom QA remains.
  - 2026-08-06 Sam-feedback correction release is production-backed. Migration `20260806120000` (`26cddf14d783a8a2b0f7bcced339357abf000a714c0585cfec07eb5615c9297a`), the JWT-protected Strategic Review checkpoint function, and exact MM repair gate `20260806130000` (`48c40b20290301ca45e889034457e9cfeb2d1c679b4435be57a5492b118ec8db`) are live. The forced-rollback proof first caught and prevented a PL/pgSQL compile defect; after correction it proved all 13 repairs, Karrie idempotency, and the canonical Aug 6 due date with zero persisted changes. The production gate then created 13 contract-linked Review Complete cards, corrected 11 shifted Daily Pulse dates, and linked Karrie to item `bd6894e0-78e2-4377-80d4-9990f50d48e3`; MM now has 223 visible cards. Renewal preview synchronizes the board to its exact custom date range and explains repeated 100-card safe batches. Daily recurrence now rechecks catch-up through lead horizon while retaining the live 25/company and 100/global caps; MM remains the sole enrolled company. Build plus 18/18 correction, 56/56 foundation, 47/47 workflow, and 27/27 rollout checks pass. `[~]` `[qa]` Jay/Sam production UI confirmation remains.
...
- `[~]` `[polish]` Clients list/card views and filters exist; Ethical Scaling now reads app-owned client rows.
  - 2026-07-17 contact-touch reliability release `9ae70c1` removes the full roster reload after each “Mark contacted today” click, keeps concurrent client touches independent, prevents duplicate same-client submits, confirms both the returned client and history action, and silently reconciles only after a burst finishes. Awaiting Adam QA before promotion. [qa]
  - 2026-07-16 navigation-continuity release `3357386` preserves the applied filter draft/result pair, page, rows per page, view, and sorting when leaving and returning; returning from Client Detail keeps the originating Clients URL, and list/card/notes/calendar client entry points are real links with right-click and Cmd/Ctrl-click support. Awaiting Jay QA before promotion. [qa]
...
- `[x]` CRUD client contracts V1.
  ...
  - 2026-07-15 MM legacy-renewal correction: 815 migrated client summaries used a `2075-01-01` placeholder despite a valid start/duration, causing incorrect roster Weeks Left values (Carol Weyrauch showed 2,529 rather than the Aug 4, 2026 renewal). Production data now uses calculated end dates; 27 matching contract-summary rows were corrected. The roster also defensively derives the date from start/duration if this legacy placeholder appears again. 116 durationless/open-ended client summaries and 24 linked open-ended contract rows remain unchanged.
...
- `[x]` Beacon assistant chat secure V1.
  ...
  - 2026-07-14 semantic completion deployed: Alima partial-name/Emily resolution passed. Ali correctly failed closed because Jay is primary on the back-end record and secondary on LBA, but the product needed primary-management semantics; Emily health incorrectly treated active as front-end only. Reversible additive `20260714022000` (`19fe74a7b5adf7ab07b2a721eeea5d6ba41e8409449b26c2455fb39b8d048eb0`) defines `activeOnly` as front-end plus back-end and maps “under/managed by” to primary CSM while generic assignment may include secondary. Authorization and ambiguity remain fail-closed. DB 83/83, Edge 43/43. Retest Ali and Emily only; Moves remains disabled. [qa]
  - 2026-07-14 final assignee-role correction deployed: Emily's health retest passed with all six active clients. Ali still failed because Jay's active account role is Director even though he is the primary assigned manager, and the partial resolver incorrectly required assignee role CSM. Reversible `20260714023000` (`1f90973afc1433cb8386809d7154b2aa6da7906d0f1f877d59dfef6c336da1b5`) derives eligible assignees only from active same-company members referenced on actor-authorized client assignments, regardless account role; CSM actors remain self-restricted. A direct production Ali/Jay-primary brief returns exactly the back-end record and saved next steps. DB 87/87, Edge 44/44. One conversational Ali smoke remains; Moves disabled. [qa]
  - 2026-07-14 brief-orchestration correction deployed: the Ali/Jay conversational smoke resolved the correct compact client row but stopped after `list_clients`, falsely treating absent compact-list fields as absent data. The reviewed tool description and system contract now require `get_client_brief` before answering summaries, north stars, next steps, or detailed contract questions; after one human-name match, orchestration must continue to the brief and may never claim absence from `list_clients`. Edge 44/44 passes. One final Ali smoke remains; Moves disabled. [qa]
  ...
  - 2026-07-16 Moves Method Director pilot is live after Jay validated role gates, counts, security refusals, and onboarding. Jay also QA-approved Beacon drag-and-dock polish: desktop/fine-pointer users can move the closed launcher or open panel header, release snaps to a viewport-safe edge, resize/open/close remain bounded, and only the dock preference persists; mobile/coarse-pointer behavior and memory-only chat remain unchanged. [qa]
...
- `[~]` `[polish]` Retention Percentage.
  ...
  - 2026-08-06 correction is live (superseding the 2026-08-04 active-only denominator): migration `20260806100000` (`af2d2f980f162c15d38a1d04709a3ff18f92d27f87699d849b15e6c17b05ba56`) restores normally completed clients when actual offboard is on/after contract end; guard `20260806103000` (`fe8ce21df5d60e6abcad3799f0363562b38bceef99258b577bcf02f2466cdc90`) explicitly excludes every churn-coded offboard, including MIA auto-offboard. Paused, Suspended/MIA, early or churn-coded offboards, undated offboards, archived rows, and analytics exclusions remain out. MM July production postflight is 334 eligible / 48 retained / 50 active unresolved / 14%, with zero expected records missing and a 1.4-second canonical read. Dean Bair is correctly retained after completing July 29 and renewing July 30. [qa]
  - 2026-08-07 successor-contract correction is production-applied through reversible migrations `20260807170000` (`4943f59024aa8de1deb407bed1ca7016d9fffb7b520a2214ca8152d5ef93fa21`), `20260807173000` (`cd5f3b06022307071b385deb4a42f888a5eba210ea46bc3e4c196632c92dc277`), `20260807174500` (`632a7545a8bb0212fa07375337473c54e92e81db11879288d1013526a8f326da`), and `20260807180000` (`9638b988e978a8488a5bcf7012dc646291444c6c97218c62d57114c943e17626`). A candidate-bound, non-add-on successor contract now counts as first-class retention evidence even when migration omitted the separate retention event; it outranks inferred legacy status evidence without letting a contract renew itself. Early successors remain in contract-end month and late successors move to successor-start month. MM July Front End/Back End changed from 86 eligible / 47 retained / 39 unresolved to 86 / 56 / 30. Don Morehouse, Danielle Mein, Caroline Lai, and Rafiq Barnes now count in July; Monica Vigil correctly moved from July to August. No customer data was mutated. Static 19/19, renewal regressions, build, and production readback pass; Jay/MM UI QA remains. [qa]
...
- `[~]` `[polish]` Define notification infrastructure.
  ...
  - 2026-07-27 MM automatic MIA offboards now create one retry-safe client task from an editable `suspended_auto_offboard` task template, assigned to the current primary coach with active Director/Support fallback. Daily Pulse keeps these follow-ups visible until the company-configured completion action, and automated History events identify RetainOS instead of leaving the actor blank. The persistent block activates only when that company has both automatic MIA offboarding and an enabled follow-up template; its title, instructions, and button text are template-configurable. The exact recent MM automation backlog produced 62 assigned tasks from 62 events with 0 creation failures and 5 audited fallbacks. Follow-up UI QA found all 62 task clients fell outside Supabase's first 1,000 returned clients, so the task/client visibility intersection hid them; Daily Pulse now explicitly loads task-linked clients before filtering. Jay/MM UI retest remains. [qa]
  - `[x]` 2026-08-05 MM MIA dropoff-label release keeps the stable `auto_suspended_timeout` churn reason, displays it as `Dropoff — MIA timeout`, and prevents future automation from overwriting company label edits. Existing automatic offboards required no client/history rewrite: all 65 reason-linked offboards remain intact, Churn Reasons resolves the company label dynamically, and Client Details/automated History expose the exit outcome and reason. Migration SHA-256 `cd72892999d024a3d85e60fd323c4f967a86ecdf04be45049798632508cc9db5` and frontend commit `05204ab` are live; Vercel and production HTTP/bundle checks passed.
  - 2026-07-28 client deletion correction: app-owned clients can have immutable Beacon assignment-history references, so hard deletion left partially archived records that disappeared from search but leaked into dashboard drilldowns. Deletion now completes as an audited archived tombstone, sets the shared dashboard exclusion, preserves immutable evidence, and blocks direct-detail fallback to stale mirror data. Shanique Messado was reconciled and verified absent from both client search and Lorcan's churn drilldown. General delete-button QA remains. [qa]
...
- `[~]` `[polish]` Dashboard chart segments are clickable and can drill into the clients behind the number where wired.
  - 2026-08-04 renewal corrections: the Active Clients Up For Renewal drawer invokes the same canonical actor-scoped cohort as its headline card (`f02f692`), and migration `20260804193000` now prunes archived, analytics-excluded, Paused, MIA, and Offboarded clients before contract/history work. Explicit-period failures show unavailable metrics instead of mixed legacy formulas. Production evidence: Vince August is 7 eligible / 1 retained / 6 active unresolved; Ben's July All-programs view now matches Front End/Back End at 98 eligible / 46 retained / 47% in under one second. Jay/Vince/Ben UI QA remains. [qa]
  - 2026-08-06 formula correction candidate preserves the accelerated actor-scoped path but restores dated normal completions to the denominator; the existing contract-level comparison still excludes early churn. No Dashboard component or route-performance code changes. [qa]
...
- `[~]` `[qa]` Company-configurable client interactions and Strategic Review orchestration.
  ...
  - 2026-08-05 Outcomes attendance is live through `fd00ace`: Calls & Reviews adds a branded Record Call form for attended/missed, company-configured call type, and optional note. It deliberately reuses `manage-client-quick-update`, so assignment permissions, attended-call last-contact behavior, History, totals, and recent evidence remain one system. Jay local save/UI QA passed, Vercel reached Ready, and the production Client Detail bundle was verified; no migration/backend deploy was required. [release]
