...
- `[~]` `[polish]` Define notification infrastructure.
  ...
  - 2026-08-05 MM MIA dropoff-label candidate keeps the stable `auto_suspended_timeout` churn reason, renames its company display label to `Dropoff — MIA timeout`, and prevents the worker from overwriting future company label edits. Existing automatic offboards need no client/history rewrite: the Churn Reasons chart resolves the company label dynamically, while Client Details and automated History expose the exit outcome and reason. Migration/build and Jay UI QA remain. [qa]
