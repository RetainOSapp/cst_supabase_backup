...
- `[~]` `[polish]` `[priority: low]` CRUD Tasks.
  ...
  - 2026-07-06 MM launch hotfix: Tasks board/list now resolves linked clients by both app-owned client UUID and legacy Glide client id, so imported tasks no longer show `Unknown client` when their `client_id` shape differs.
