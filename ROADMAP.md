...
- `[~]` `[qa]` Call Intelligence V1 local release candidate.
  ...
  - Automated evidence passes: Edge/provider/dispatch 18/18, eval scorer/harness 4/4, DB/dependency/rollback contract 54/54, Edge/source security 46/46, frontend/security/privacy 19/19, production build, and browser QA at 1440px/320px. Official OpenAI docs confirm Luna/Terra/Sol API identifiers, strict Responses structured output including string `pattern`, and Terra-medium as the balanced starting point. Model-scoped `openai-standard-2026-07-23` pricing is pinned; calls force standard tier and disable implicit cache writes.
  ...
  - Evidence-v2 correction is local and green: `structured_v2_evidence_v2` sends an explicit sanitized speaker-role map, forces unmatched/ambiguous speakers to remain `unknown`, machine-enforces 4–12 whitespace-separated quote words with strict-schema `pattern`, keeps timestamp/role fail-closed validation, and mechanically synchronizes the migration seed. The focused retest is five structured-only calls with a conservative 1,123,498-micro ($1.13 rounded) ceiling; it has not run. Production remains unchanged. [qa]
