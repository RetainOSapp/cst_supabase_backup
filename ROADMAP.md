...
- `[~]` `[qa]` Call Intelligence V1 local release candidate.
  ...
  - 2026-07-24 disabled production Phases A/B are live: both additive migrations applied with recorded hashes; six tables and quality-v4 prompts read back; all three functions are active with correct JWT configuration; invalid/missing token and anonymous probes deny. Global `call_analysis` remains paused with zero entitlements, allowances, tokens, customer records, usage, and provider spend. Hidden frontend release is next. [qa]
