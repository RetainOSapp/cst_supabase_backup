...
- `[~]` `[qa]` Call Intelligence V1 local release candidate.
  ...
  - Next QA gate requires fresh approval: three eligible quality-v3 Terra-medium calls plus one synthetic adversarial call, with a combined 936,559-micro ($0.94 rounded) dry-run ceiling, followed by private output review. Only then consider the separately approved disabled DB/function/frontend pilot: one known-client call, replay/dedupe, unknown and multi-client reconciliation, structured output, one on-demand prompt, role scope, hard-budget/global-pause denial, and rollback.
