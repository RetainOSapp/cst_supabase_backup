...
- `[~]` `[polish]` `[priority: low]` CRUD Tasks.
  ...
  - 2026-07-06 MM launch hotfix: Tasks board/list now resolves linked clients by app-owned client UUID, legacy Glide client id, known imported metadata keys, and the loaded company roster, so imported tasks stop showing `Unknown client` when their `client_id` shape differs.
